<template>
  <v-carousel height="100vh" hide-delimiter-background>
    ...
    <v-carousel-item v-for="(p,index) in sale_item" :key="`setItem-${index}`">
...
<p class="text-subtitle-1 text-md-h5 primary--text mt-5"> {{ p.price }}</p>
<p class="text-md-body-2 mb-7"> {{  p.description }}</p>
<v-btn depressed color="primary" class="text-capitalize" min-height="40"> Check It Out </v-btn>
              </div>
            </v-col>
          </v-row>
        </v-container>
